...
CIPHERS = 'LEGACY-GOST2012-GOST8912-GOST8912'  # API's requirement
BASE_URL = 'https://api.mdlp.crpt.ru/api/v1/'   # Base url for post requests

CA = ''  # trusted Certificate Authorities (insert path to .pem file)
URL = 'https://api.mdlp.crpt.ru'  # API MDLP Url

CLIENT_ID = ''  # Accounting System (insert from your personal account)
CLIENT_SECRET = ''  # Accounting System (insert from your personal account)
USER_ID = ''  # User's certificate's print (insert)

PATH_TO_DIRECTORY_WITH_XML = ''  # path to directory where xml files to sign and send are (insert)
CERT_PIN = ''  # the PIN code of private certificate (insert)
...
def main():
    ...
    with open('unloaded.csv', 'w', newline='') as csvfile:
    ...
    for filename in os.listdir(PATH_TO_DIRECTORY_WITH_XML):
        ...
        if os.path.isfile(path_to_xml_file) and os.path.splitext(filename)[1] == ".xml":
            ...
            if error:
                with open('unloaded.csv', 'a', newline='') as csvfile:
                        fieldnames = ['filename', 'error']
                        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
